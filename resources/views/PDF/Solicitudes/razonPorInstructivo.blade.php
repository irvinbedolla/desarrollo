                ...
                <div class="razon-notificacion"> 
                    ...
                    <p>Siendo las <b>{{ $fechaNotificacion ? $fechaNotificacion->format('H') : '' }} HORAS CON {{ $fechaNotificacion ? $fechaNotificacion->format('i') : '' }} MINUTOS
                        ...
                        {{-- CUANDO TE ATIENDEN, PERO NO QUIEREN RECIBIR --}}
                            ...
                            <b>{{$citado->nombre_notificacion}}</b>@else <b>SE NIEGA A PROPORCIONAR SU NOMBRE</b>@endif <b>QUIEN @if($citado->identificacion_notificacion != null) SE IDENTIFICA CON {{mb_strtoupper($citado->identificacion_notificacion, 'UTF-8')}} de número {{$citado->num_identificacion}}@else NO SE IDENTIFICA ALEGANDO {{mb_strtoupper($citado->motivo_identificacion, 'UTF-8')}}@endif.</b>
                            Procedo a especificar su media filiación, que incluye los siguientes rasgos:<b>SEXO {{$citado->sexo}}, TEZ {{ $citado->tez }}, EDAD {{$citado->edad}}, ALTURA {{$citado->altura}} M, COMPLEXIÓN {{$citado->complexion}}, CABELLO {{$citado->cabello}} Y OJOS {{$citado->ojos}}.</b>
                            <b>LO ANTERIOR SE HACE DE MANERA APROXIMADA, YA QUE EL SUSCRITO NO ES PERITO EN LA MATERIA</b>. Quien manifiesta que @if($citado->puesto=="TRABAJA")<b>OCUPA EL PUESTO DE {{$citado->puesto}}</b>@endif @if($citado->puesto=="RECIDE")<b>{{$citado->puesto}}</b>@endif  

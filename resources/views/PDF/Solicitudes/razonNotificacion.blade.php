            ...
            <div class="content">
                ...
                <div class="razon-notificacion">
                    ...
                    <p>Asimismo, por los informes que me proporciona la persona con quien se entiende la presente diligencia, quien dijo llamarse <b>{{ $citado->nombre_notificacion }}</b>, <b>QUIEN SE IDENTIFICA CON {{$citado->identificacion_notificacion}}</b> de número <b>{{$citado->num_identificacion}}.</b>
                        Quien manifiesta que @if($citado->relacion_notificacion=="TRABAJA")<b>OCUPA EL PUESTO DE {{$citado->puesto}}</b>@endif @if($citado->relacion_notificacion=="RECIDE")<b>{{$citado->puesto}}</b>@endif en el domicilio en que se actúa. Enseguida me identifico con credencial vigente expedida por el Centro de Conciliación Laboral, oficina estatal {{ mb_strtoupper($solicitud->delegacion,'UTF-8') }} que me acredita 
                    ...
                    <p>Asimismo, por los informes que me proporciona la persona con quien se entiende la presente diligencia, quien dijo llamarse <b>{{ $citado->nombre_notificacion }}</b>, <b>QUIEN NO SE IDENTIFICA ALEGANDO QUE {{$citado->motivo_identificacion}}.</b> 
                        ...
                        LO ANTERIOR SE HACE DE MANERA APROXIMADA, YA QUE EL SUSCRITO NO ES PERITO EN LA MATERIA.</b> Quien manifiesta que @if($citado->relacion_notificacion=="TRABAJA")<b>OCUPA EL PUESTO DE {{$citado->puesto}}</b>@endif @if($citado->relacion_notificacion=="RECIDE")<b>{{$citado->puesto}}</b>@endif en el domicilio en que se

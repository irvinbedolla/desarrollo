    ...
    <body>
        ...
        <main>
            <div class="content">
                ...
                <p>
                    SOLICITANTE: <b>{{ $solicitante->nombre }}</b><br>
                    CITADO(S):
                    <b>
                ...
                <p>Con fundamento en los artículos 123, apartado A, fracciones XX párrafo segundo y XXVII, inciso h) de la Constitución Política de los Estados Unidos Mexicanos;
                    ...
                    <b>{{ $solicitante->nombre }}</b> quién en lo subsecuente se denominará la parte <b>“TRABAJADORA”</b> y, por otro
                    <b>@foreach($citados as $citado) {{ $citado->nombre }} {{ $citado->primer_apellido }} {{ $citado->segundo_apellido }},@endforeach</b>
                ...
                <p><b>SEGUNDA</b>.
                    ...
                    @foreach($listaAbogados as $idx => $rep)
                        @php
                            ...
                            $nombrePatronal = trim(($rep->nombres_patronal ?? '').' '.($rep->primer_apellido_patronal ?? '').' '.($rep->segundo_apellido_patronal ?? ''));
                        ...
                        @if(!$tieneRepresentante)
                            La parte <b>EMPLEADORA</b> <b>{{ $nombrePatronal }}</b> quien se identifica con <b>{{ mb_strtoupper($rep->tipo_identificacion ?? '', 'UTF-8') }}</b> expedida a su favor por <b>{{ $descId }}</b>, declara ser una persona mayor de edad, por lo que tiene plenas capacidades de goce y ejercicio para convenir o transigir.
                        @else
                            Declara <b>{{ $nombreRepresentante }}</b> quien se identifica con <b>{{ mb_strtoupper($rep->tipo_identificacion ?? '', 'UTF-8') }}</b>, que es apoderado legal de <b>{{ $nombrePatronal }}</b> y que cuenta con facultades suficientes para
                            convenir a nombre de su representada en términos de <b>{{ $rep->descipcion_poder }}</b>, facultad que a la fecha de este convenio no le ha sido revocado.
                        @endif
                    @endforeach
                </p>
                ...
                <p><b>TERCERA</b>. Declara la parte <b>TRABAJADORA</b>:</p>
                    <p class="sangria">
                        a) Que fue contratada por la parte <b>EMPLEADORA</b> desde el <b>{{ \Carbon\Carbon::parse($solicitante->fecha_ingreso)->translatedFormat('d \d\e F \d\e\l Y') }}</b>, para prestar sus servicios como <b>{{ $solicitante->puesto }}</b>, @if(!$solicitante->fecha_salida) puesto en el que actualmente se desempeña</b>@else , puesto en el que se desempeñó hasta el día <b>{{ \Carbon\Carbon::parse($solicitante->fecha_salida)->translatedFormat('d \d\e F \d\e\l Y') }}</b>@endif.
                    </p>
                    <p class="sangria">
                    ...
                    <p class="sangria">
                        c) Que el día <b>{{ \Carbon\Carbon::parse($solicitud->fecha)->translatedFormat('d \d\e F \d\e\l Y') }}</b> presentó solicitud para iniciar el procedimiento de conciliación prejudicial ante el Centro de Conciliación Laboral del
                        Estado de Michoacán de Ocampo, por motivo de <b>Pago de prestaciones</b>, consistente en el <b>reparto de utilidades correspondientes al ejercicio fiscal {{ $datosAudiencia->year_ptu }}</b>,
                    ...
                    <p class="sangria">
                        d) Que el Centro Estatal, fijó la audiencia de conciliación para el día <b>{{ \Carbon\Carbon::parse($audiencia->fecha)->translatedFormat('d \d\e F \d\e\l Y') }}.</b>
                    </p>

                <p> <b>CUARTA.</b> Declara la parte <b>EMPLEADORA:</b> </p>
                    <p class="sangria">     
                        a) Que reconoce adeudo a favor de la parte <b>TRABAJADORA</b> por concepto de Participación de los Trabajadores en las Utilidades (PTU) correspondiente al ejercicio fiscal <b>{{ $datosAudiencia->year_ptu }}</b>.
                    </p>
                    <p class="sangria">
                    ...
                    <p class="sangria">
                        ...
                        con el fin de determinar la cantidad que corresponde a los trabajadores de la <b>EMPLEADORA</b> por concepto de reparto de utilidades del ejercicio fiscal <b>{{ $datosAudiencia->year_ptu }}</b>.
                    </p>
                    <p class="sangria">
                    ...
                    <p>
                        <b>PRIMERA</b>. El presente convenio tiene por objeto fijar la forma y términos en que la parte <b>EMPLEADORA</b> cubrirá a la parte <b>TRABAJADORA</b> el pago correspondiente por concepto de Participación de los Trabajadores en las Utilidades (PTU) del ejercicio fiscal <b>{{ $datosAudiencia->year_ptu }}</b>.<br><br>
                        ...
                        <b>TERCERA</b>. La parte <b>EMPLEADORA</b> manifiesta bajo protesta de decir verdad, que la cantidad pactada en el presente convenio corresponde al monto determinado conforme al proyecto de reparto aprobado por la Comisión Mixta para la Participación de los Trabajadores en las Utilidades de la empresa,
                         correspondiente al ejercicio fiscal <b>{{ $datosAudiencia->year_ptu }}</b>. <br><br> Asimismo, manifiesta que dicho monto fue calculado conforme a la información fiscal y laboral aplicable, asumiendo plena responsabilidad respecto de su determinación, procedencia y exactitud. <br><br>
                        ...
                        <b>QUINTA.</b> La parte <b>TRABAJADORA</b> recibirá de la <b>EMPLEADORA</b> la cantidad de <b>${{ number_format($datosAudiencia->monto, 2) }} {{ $montoTexto }}</b>, conforme a los siguientes conceptos:<br><br>
                        ...
                        <table class="table table-bordered table-compacta" style="width:100%;">
                            <thead>
                            <tr>
                        ...
                        <b>SÉPTIMA.</b> Las <b>PARTES</b> solicitan se apruebe y sancione el presente convenio que únicamente regula lo relativo al pago de <b>Participación de los Trabajadores en las Utilidades de la empresa PTU del ejercicio fiscal {{ $datosAudiencia->year_ptu }}</b>,
                        ...
                        ejercitar con posterioridad a la firma del presente convenio por lo que ve a la Participación de los Trabajadores en las Utilidades de la empresa citada (PTU) del ejercicio fiscal <b>{{ $datosAudiencia->year_ptu }}</b>.<br><br>
                    ...
                    <div class="contenedor-firmas">
                        <p>
                            Enteradas las <b>PARTES</b> del alcance legal del presente convenio que se eleva a cosa juzgada, conforme al artículo 684-E fracción XIII, mismo que se firma en <b>{{ $solicitud->delegacion }},  Michoacán de Ocampo a {{ \Carbon\Carbon::parse($audiencia->fecha)->translatedFormat('d \d\e F \d\e\l Y') }}</b>,
                        ...
                        <p style="font-size: 10px;">
                            LAS PRESENTES FIRMAS FORMAN PARTE INTEGRA DEL CONVENIO DE CONCILIACIÓN DE PAGO DE PARTICIPACIÓN DE UTILIDADES DE FECHA <b>{{ \Carbon\Carbon::parse($solicitud->fecha)->translatedFormat('d \d\e F \d\e\l Y') }}</b> EXPEDIENTE NÚMERO <b>{{ $solicitud->NUE }}</b> DEL CENTRO DE CONCILIACIÓN LABORAL DEL ESTADO DE MICHOACÁN DE OCAMPO.
                        </p>
                    </div>           
            </div>

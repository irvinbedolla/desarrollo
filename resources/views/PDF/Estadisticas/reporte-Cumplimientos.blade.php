    ...
    <body>
        ...
        <main>
            <table class="header-info-table">
                <tr>
                    <td class="header-title">Reporte de Cumplimientos Laborales</td>
                    <td class="header-date">
                        <strong>Periodo:</strong> 
                        {{ \Carbon\Carbon::parse($fecha_inicial)->format('d/m/y') }} - {{ \Carbon\Carbon::parse($fecha_final)->format('d/m/y') }}
                    </td>
                </tr>
            </table>
            ...
            <div class="section-title">Cumplimientos en Ratificaciones</div>
                ...
                <tbody>
                    @foreach($pagosRatificacion as $estadistica)
                        <tr>
                            ...
                            <td class="monto-bold">${{ number_format($estadistica->monto, 2) }}</td>
                            <td>{{ $estadistica->delegacion }}</td>
                            <td>{{ $estadistica->conciliador_name }}</td>
                            <td><span class="estatus-pill">{{ $estadistica->estatus }}</span></td>
                        </tr>
                        @php $totalRatificacion += $estadistica->monto; @endphp
                    @endforeach
                </tbody>
                <tfoot>
                    <tr class="row-total">
                        <td colspan="6" class="total-label">TOTAL RATIFICACIONES:</td>
                        <td colspan="4" style="text-align: left; padding-left: 10px;">
                            ${{ number_format($totalRatificacion, 2) }}
                        </td>
                    </tr>
                </tfoot>
            </table>

            <div class="section-title">Cumplimientos en Audiencias</div>
            <table class="table-custom">
                <thead>
                    <tr>
                        <th>Fecha</th>
                ...
                <tbody>
                    @foreach($pagosAudiencias as $estadistica)
                        <tr>
                            ...
                            <td>{{ date_format($estadistica->hora, 'H:i') }}</td>
                            <td style="font-weight:bold;">{{ $estadistica->NUE }}</td>
                            <td>{{ $estadistica->empresa_representante }}</td>
                            <td>{{ $estadistica->nombre_trabajador }}</td>
                            <td>{{ $estadistica->descripcion }}</td>
                            <td class="monto-bold">${{ number_format($estadistica->monto, 2) }}</td>
                            <td>{{ $estadistica->delegacion }}</td>
                            <td>{{ $estadistica->conciliador_name }}</td>
                        ...
                        @php $totalAudiencias += $estadistica->monto; @endphp
                    @endforeach
                </tbody>
                <tfoot>
                    <tr class="row-total">
                        <td colspan="6" class="total-label">TOTAL AUDIENCIAS:</td>
                        <td colspan="4" style="text-align: left; padding-left: 10px;">
                            ${{ number_format($totalAudiencias, 2) }}
                        </td>
                    </tr>
                </tfoot>
            </table>
            ...
            <div class="resumen-container">
                <table class="resumen-table">
                    ...
                    <tbody>
                        <tr>
                            <td>Total Ratificaciones:</td>
                            <td align="right"><strong>${{ number_format($totalRatificacion, 2) }}</strong></td>
                        </tr>
                        <tr>
                            <td>Total Audiencias:</td>
                            <td align="right"><strong>${{ number_format($totalAudiencias, 2) }}</strong></td>
                        </tr>
                        <tr class="resumen-grand-total">
                            <td>GRAN TOTAL:</td>
                            <td align="right">${{ number_format($totalRatificacion + $totalAudiencias, 2) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
